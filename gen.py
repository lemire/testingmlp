...
for i in range(1,100):
    print("uint64_t guarded_naked_access_%d(uint64_t *bigarray, size_t howmanyhits) {"%(i))
    for j in range(1,i+1):
        print("  uint64_t val%d = %d;" %(j,j))
    print ("  size_t howmanyhits_perlane = howmanyhits / %d;"%(i))
    print("  for (size_t counter = 0; counter < howmanyhits_perlane; counter++) {")
    for j in range(1,i+1):
        print("    if(val%d >= howmanyhits) abort();" %(j))
        print("    val%d = bigarray[val%d];" %(j,j))
    print("  }")
    ret = "  val1 "
    for  j in range(2,i+1):
...
print("void naked_measure_body(float (&time_measure)[NAKED_MAX], float (&guarded_time_measure)[NAKED_MAX], uint64_t *bigarray, size_t howmanyhits, size_t repeat) {")
for i in range(1, 100):
    ...
    print("  guarded_time_measure[%d] = time_one(bigarray, howmanyhits, repeat, naked_access_%d, %d, %s, %s);"%(i,i,i,first,last))
print("}")
